...
namespace ShareX.MediaLib
{
    public class FFmpegCLIManager : ExternalCLIManager
    {
        ...
        protected bool Run(string path, string args)
        {
            ...
            if (!result && ShowError)
            {
                DebugHelper.WriteLine(Output.ToString());
            }
            return result;
        }
        ...
        public VideoInfo GetVideoInfo(string videoPath)
        {
            ...
            if (matchVideoStream.Success)
            {
                ...
                videoInfo.VideoResolution = new System.Drawing.Size(int.Parse(matchVideoStream.Groups["Width"].Value), int.Parse(matchVideoStream.Groups["Height"].Value));
